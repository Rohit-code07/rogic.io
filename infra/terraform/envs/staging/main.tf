...
resource "aws_iam_role" "github_actions_staging" {
  name = "nemologic-staging-github-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          ...
          StringLike = {
            "token.actions.githubusercontent.com:sub" = "repo:devdoyen/rogic.io:*"
          }
        }
      }
    ]
...
resource "aws_iam_policy" "github_actions_staging_policy" {
  name        = "nemologic-staging-github-policy"
  description = "Custom policy for staging GitHub Actions runner with least privilege"
