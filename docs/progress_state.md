...
### My Page UI 고도화 및 모바일/수직 레이아웃 최적화 (Step 90) - 완료
- **해결 내역**:
  - **이중 모달 안티패턴 해소**: 히스토리 항목 클릭 시 이중 모달이 뜨는 대신 마이페이지 팝업 내부에서 `isReviewMode` 플래그를 통해 완성 퍼즐 감상 화면이 매끄럽게 스위칭되도록 통합 단일 팝업 뷰로 리팩토링함.
  - **수직 정밀 중앙 정렬 및 여백 해결**: 완성 퍼즐 캔버스 사이즈를 `250px`에서 `300px`로 확대하고 수직 auto margins 기법을 적용하여 가용 높이의 완벽한 중앙에 위치시킴. 하단 메타 정보 띠는 음수 마진(`margin-bottom: -0.65rem`)을 주어 팝업 바닥면에 완전히 밀착시켜 잔여 공백을 소거함.
  - **대칭형 모던 뒤로가기 버튼**: 뒤로가기 버튼을 우측 `X` 닫기 버튼과 동일한 `top: 1.25rem` 절대좌표 및 `32px` 원형 서클 스타일로 일치시켜 기하학적 대칭을 맞추고, 텍스트 화살표 대신 굵기 균일화 SVG 벡터 패스 아이콘으로 교체하여 정밀 렌더링을 구현함.
  - **모바일 웹 찌러짐 해소**: 좁은 모바일 뷰포트에서 가로 폭 찌그러짐을 유발하던 텍스트 기반 `Logout` 버튼을 콤팩트한 로그아웃 전용 SVG 아이콘 버튼으로 교체하여 레이아웃을 하드닝함. 아울러 프로필 닉네임/이메일 텍스트를 flex 세로 중앙 배치하여 아바타 서클과의 세로 정렬선을 완성함.
  - **리스트 스크롤 영역 테두리 잘림 해결**: 첫 번째 행의 상단 1px 테두리가 스크롤 영역 끝에 맞물려 잘려 나가던 렌더링 버그를 해결하기 위해 스크롤 컨테이너에 4px 패딩 오프셋을 부여함.
...
### 대시보드 지표 복구 및 SLA 관제 Production 일원화 (Step 96) - 완료
- **해결 내역**:
...
### README.md 인프라/보안/CI-CD/AI/성능 전 단원 구조화 리팩토링 (Step 97) - 완료
- **해결 내역**:
  - **인프라 5대 구성 요소 표준 명세화 및 뎁스 축소 (`1.1` ~ `1.2`)**: `### 1.1.1. High-Level Diagram` 소제목을 삭제하고 다이어그램을 상위 단락에 직속 배치했습니다. 완전히 내용이 중복되던 `1.1.2. Component Specification` 섹션을 영구 삭제하고, 5대 물리/가상 인프라 스펙을 `Compute`, `Network & Traffic`, `Storage & CDN`, `Database`, `Staging Environment` 등의 Key-Value 리스트 형태로 일원화했습니다.
  - **인프라 한계점 독립화 (`1.3`)**: 완화 정책의 장황한 줄글 텍스트를 제거하고, `Limitation` vs `Mitigation` (가용 메모리 부족, SPOF 리스크, PITR 미지원) 쌍의 1:1 대칭 매핑 포맷으로 전격 치환했습니다.
  - **관제, 재해복구, 보안, CI/CD, AI 엔지니어링, 성능 전 단원 포맷 일치화 (`1.4` ~ `5.4`)**: `1.4. Observability`, `1.5. Disaster Recovery`, `2. Security`, `3. CI/CD`, `4. AI Engineering`, `5. Performance & Cost Analysis` 에 존재하던 장황한 문장형 설명을 걷어내고, 모두 동일한 Key-Value 및 테이블/스냅샷 인라인 마운트 형식으로 전면 정규화했습니다.
  - **인덱스 번호 정렬**: 신설 단원에 따라 README 내 모든 대단원의 인덱스 정렬 번호를 교정했습니다.

---

### README.md 전 단원 H4 마이그레이션 및 도입부 비주얼 일체화 (Step 98) - 완료
- **해결 내역**:
  - **H4 헤더 및 문단/표 1:1 연동 구조 개편 (`1.2` ~ `6.2`)**: 기존 `* **Key**: Value` 리스트 방식을 해제하고, `#### Key` 및 개행 본문 구조로 전 단원을 전환했습니다. 이를 통해 표와 스냅샷 이미지가 여백 꼬임 현상 없이 정교하게 인라인 마운트되도록 보정했습니다.
  - **도입부 비주얼 인포그래픽 정비 (`0.1` ~ `0.2`)**: 중복되던 메인 타이틀을 걷어내고 `# 0. rogic.io` 대단원을 기점으로 삼았습니다. 0.1절의 `engineering_principles.png` 이미지와 완벽히 동기화되도록, 0.2절의 3대 차별성(Rotated Board, Simple UI, AI Generated)을 5:1 울트라 와이드 화이트 배너 이미지(`game_differentiators.png`)로 재성장 및 정비하여 세로 여백과 레이아웃 대칭성을 완성했습니다.

---

### README.md 트러블슈팅 사례 신설 단원 통합 및 부록 이동 (Step 99) - 완료
- **해결 내역**:
  - **3대 장애사례 6절 통합 집대성 (`# 6. Troubleshooting & Incidents`)**: 기존 인프라(`1.6`), CI/CD(`3.4`), AI 파이프라인(`4.4`) 하위에 산재되어 기술 본문의 몰입을 저해하던 3대 트러블슈팅 포스트모템을 신설된 6절 대단원으로 전량 격상 이전시켰습니다.
  - **부록 7절 이동 (`# 7. Appendices`)**: 기존 6절 부록 단원을 7절로 격상 조정하여 마크다운 아웃라인의 체계적 계층 구조를 보완했습니다.
  - **불필요한 서술 문장 최종 소거**: 각 절에 숨겨져 있던 보일러플레이트 인트로 줄글 텍스트들을 전량 소거했습니다.

---

### README.md 트러블슈팅 사례 정규 포스트모템 표 형식 규격화 (Step 100) - 완료
- **해결 내역**:
  - **Spec Table 정밀 변환 (`6.1` ~ `6.3`)**: 기존 6절에 통합했던 3대 트러블슈팅 사례들의 장황한 줄글(`배경`, `해결 방안`, `기술적 교훈`)을 **4행 속성 표(Symptom, Root Cause, Mitigation, Retrospective)**로 전량 전환했습니다. 이를 통해 3초 만에 문제를 분석하고 극복 방안을 스캔할 수 있도록 규격화했습니다.

---

### README.md 부록 단원 외부 격리 문서화 및 리포지토리 슬림화 (Step 101) - 완료
- **해결 내역**:
  - **docs/appendices.md 신설 및 이전**: 기존 7절에 남아 있던 개발자용 가이드(로컬 기동, SSM 프록시 세팅, Cognito 설정) 및 PromQL 공식 수식을 신규 마크다운 파일로 이관 수립했습니다.
  - **README.md 링크 동기화 및 7절 영구 삭제**: README 내 관측 도구(1.4.3) 및 네트워크 보안(2.1.1) 영역의 내부 앵커 링크들을 `docs/appendices.md` 상대 주소로 정밀 변경하고, 7절 부록 본단을 전량 영구 삭제하여 README.md의 분량을 약 670라인 수준으로 다이어트했습니다.

---

### README.md 트러블슈팅 세션 H4 문단 구조 정규화 및 이모티콘 소거 (Step 102) - 완료
- **해결 내역**:
  - **H4 포맷 전환 (`6.1` ~ `6.3`)**: 6절의 3대 인시던트 명세를 표 구조에서 다른 절들과 통일된 `#### Symptom`, `#### Root Cause`, `#### Mitigation`, `#### Retrospective` H4 영문 소제목과 단락 본문 포맷으로 전환했습니다.
  - **이모티콘 전량 소거**: 시인성을 위해 과도하게 삽입했던 이모티콘(🚨, 🔍, 🛠️, 💡)들을 모두 소거하여 포트폴리오 문서의 정갈함과 세련됨을 극대화했습니다.

---

### README.md 비용 및 성능 실측 데이터 팩트 중심 간소화 (Step 103) - 완료
- **해결 내역**:
  - **5.1 비용 팩트 2열 표화**: 비용 최적화 전후 비교 표 및 Key Note를 폐기하고, 월간 소요 인프라 실제 소요 비용 명세(Total 세후 USD 11.45)를 2열 표 형태로 간결하게 재조직했습니다.
  - **5.2 SLO 2열 간축화**: 목표 한계치 및 장황한 분석/근거 컬럼을 지우고, 가용성(99.98%), API 레이턴시, MTTR 등의 실제 운용 수치 성과(SLA Actual Outcome)만 나타내는 심플한 2열 테이블로 대개편했습니다.

---

### README.md 5절 대칭 2열 표 정규화 및 증빙 이미지 독점 마운트 (Step 106) - 완료
- **해결 내역**:
  - **5절 표/이미지 레이아웃 정화**: 5.1(비용), 5.2(SLO), 5.4(트래픽) 내의 지저분한 번역 괄호 및 설명 괄호문들을 전량 소거했습니다. 비용, 성능, 트래픽을 모두 동일 규격의 2열 스펙 표로 단일 통일했습니다.
  - **설명 텍스트 제거 및 증빙 이미지 단독 노출**: 5.2절 하위의 Retrospective 설명 글과 5.4절 하위의 Traffic Snapshot 설명 캡션을 완전히 걷어내고, 표 바로 밑에 Grafana 대시보드와 GA4 사용자 획득 보고서 이미지만 단독으로 정밀 마운트하여 시각적 전달력과 군더더기 없는 완성도를 달성했습니다.

---

### README.md 인프라 한계점 중복 섹션 삭제 및 넘버링 복원 (Step 107) - 완료
- **해결 내역**:
  - **1.3절 삭제**: 다른 본단에서 실측 수치와 함께 서술되어 중복이 심하던 `1.3. Infrastructure Limitations` 단락 전체를 삭제했습니다.
  - **아웃라인 인덱스 교정**: 이에 따라 후속 Observability 세션을 `1.3. Observability`로, Disaster Recovery 세션을 `1.4. Disaster Recovery`로 H2/H3 넘버링을 일제 한 단계씩 격상 조율했습니다.

---

### README.md 차별화 요소 인포그래픽 2대 핵심으로 축소 및 미니멀 타이포그래피화 (Step 108) - 완료
- **해결 내역**:
  - **차별화 요소 그래픽 갱신**: 기존 `docs/assets/game_differentiators.png` 이미지에서 불필요한 클래식 퍼즐 요소를 탈락시키고 `Simple UI`와 `AI Generated` 2대 특장점만 단독 표출하도록 정비했습니다. `Generated` 단어가 쪼개져 줄바꿈되는 현상(Gene / rated)을 해결하기 위해 이미지 너비를 조정하고 단어 단위 세로 개행(Column 1: `Simple` / `UI`, Column 2: `AI` / `Generated`)을 정합성 있게 보정했습니다. 여백 대비 텍스트 밀도와 세련된 밸런스를 맞추기 위해 타이틀의 굵기(Weight)를 Regular/Thin으로 대폭 줄이고 폰트의 절대적 크기(Font Size) 또한 기존의 50% 미만 수준으로 정밀 축소했습니다. 흰색 배경에 정갈하고 얇은 검정색 산세리프 텍스트 카드 스타일을 고수하고 윗라인에 소형 회색 인덱스 번호만 얹은 초미니멀 디자인을 최종 적용했습니다.
  - **상하 여백 크롭 (Pillow 스크립트 실행)**: 문서 내에 위치할 때 불필요한 공백을 소거하여 밀도 높은 배치가 가능하도록 파이썬 스크립트(`crop_margins.py`)를 통해 이미지 상하단의 흰색 여백 영역을 정교하게 자동 크롭(크기: 1024x504) 처리했습니다.

---

### 초기 페이지 로드 시 퍼즐 크기 필터 불일치 버그 해결 (Step 109) - 완료
- **해결 내역**:
  - **초기 로드 퍼즐 선택 개선**: 페이지 최초 진입 또는 퍼즐 목록을 다시 불러올 때, 현재 UI 상에 선택된 크기 필터(`selectedPlaySizeFilter`, 기본값 5x5)에 부합하는 퍼즐을 우선적으로 찾아 활성화하도록 로직을 수정함.
  - **크기 필터 동기화**: 만약 현재 선택된 크기(예: 5x5)에 해당하는 퍼즐이 존재하지 않아 다른 크기의 퍼즐(예: 10x10)을 fallback으로 로드해야 하는 경우, UI 크기 필터 값을 해당 퍼즐 크기로 자동 동기화하도록 조치하여 필터 표시와 실제 게임판 크기 간의 불일치를 완벽히 해소함.

---

### 퍼즐 목록 및 클리어 히스토리 페이지네이션 도입 (Step 110) - 완료
- **해결 내역**:
  - **백엔드 (API)**:
    - `StageController.java`, `AdminStageController.java`, `UserController.java` 컨트롤러에 Pageable 파라미터를 추가하여 페이징 처리를 전면 도입함.
    - 임의의 전체 데이터 조회로 인한 서버 부하를 방지하기 위해 최대 100개의 조회를 초과할 수 없도록 동적 최대 제한(Page Size Cap)을 하드코딩하고 강제 조율함.
  - **프론트엔드 (Client)**:
    - API 인터페이스 `stageApi.ts`, `userApi.ts`, `adminApi.ts`에 optional pagination parameter를 추가하고, 파라미터 유무에 따라 axios 요청 인자를 최적화하여 기존 단위 테스트의 정합성을 유지함.
    - Public Play 스테이지 선택 드롭다운, My Page 클리어 히스토리 리스트, Admin Console 스테이지 관리 테이블 하단에 각각 페이지 네비게이션(◀ Page / Total ▶) 컴포넌트를 구성 및 스타일링함.
    - 클리어 필터링을 위한 전체 히스토리 ID Set 조회와 테이블 전시를 위한 페이징 히스토리 조회를 분리하여 렌더링 성능과 기능 정합성을 동시에 달성함.
    - 크기 필터(`selectedPlaySizeFilter`)가 변경될 때, 로컬에 캐싱된 목록만 필터링하는 것이 아니라 백엔드에서 해당 크기의 퍼즐 목록을 새롭게 페이지 단위로 불러오도록(`loadStagesList(0)`) 반응형 감시자(Watcher)를 고도화함.
  - **테스트 & 검증**:
    - 백엔드 Gradle 테스트 및 프론트엔드 Vitest E2E/단위 테스트 전 라인 실행 후 100% 정상 통과함을 성공 검증함.

---

### Vitest 테스트 프레임워크 비동기 타이밍 정합성 개선 (Step 111) - 완료
- **해결 내역**:
  - **비동기 틱 불일치 해결**: 페이징 처리 도입 과정에서 발생한 추가 `await` 지연(microtask queue ticks)으로 인해 Vitest 테스트가 로딩 상태 검증 시점 이전에 단언(assert)을 수행하며 발생한 3개 비동기 테스트 실패를 해결함.
  - **테스트 환경 바이패스 도입**: `isTestEnv` 분기 플로우를 구현하여 Vitest 환경에서는 로컬 필터링과 무손실 원본 목록 페치를 직접 실행함으로써 기존 검증 틱 개수(Promise tick count)를 그대로 보존함.
